...
func (ns *nodeServer) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	if isMounted(target) {
		...
		klog.V(2).Infof("NodePublishVolume: mount %s at %s failed", source, target)
	}
...
func (ns *nodeServer) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	if err != nil {
		if err := Unmount(" -l " + targetPath); err != nil {
			if err := Unmount(" -f " + targetPath); err != nil {
				return nil, status.Errorf(codes.Internal, "failed to unmount -l -f target %q: %v", targetPath, err)
			}
			klog.V(2).Infof("NodeUnpublishVolume: unmount -f volume %s on %s successfully", volumeID, targetPath)
		}
		klog.V(2).Infof("NodeUnpublishVolume: unmount -l volume %s on %s successfully", volumeID, targetPath)
	}
